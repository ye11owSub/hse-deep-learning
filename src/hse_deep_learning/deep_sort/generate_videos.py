...
def parse_args():
    ...
    parser.add_argument("--result_dir", help="Path to the folder with tracking output.", required=True)
    parser.add_argument(
        "--output_dir", help="Folder to store the videos in. Will be created " "if it does not exist.", required=True
    )
    parser.add_argument("--convert_h264", help="If true, convert videos to libx264 (requires " "FFMPEG", default=False)
    parser.add_argument(
        "--update_ms",
...
if __name__ == "__main__":
    ...
    for sequence_txt in os.listdir(args.result_dir):
        ...
        show_results.run(sequence_dir, result_file, False, None, update_ms, video_filename)
